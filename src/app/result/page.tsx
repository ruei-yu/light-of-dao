...
const ICONS: Record<Key, { src: string; color: string; note: string }> = {
  安心之光: { src: '/icons/peace.png',   color: '#5bc0be', note: '把心安住在光中，你就能在變動中保持穩定與清明。' },
  力行之光: { src: '/icons/action.png',  color: '#ff7a59', note: '從小行動開始，願力在每一步中被喚醒，路也因此顯現。' },
  洞察之光: { src: '/icons/insight.png', color: '#6f5bd3', note: '看見因與果，心就少了糾結，多了方向與智慧。' },
  圓融之光: { src: '/icons/harmony.png', color: '#f59e0b', note: '不必硬碰硬，願你在理解中包容，在包容中前進。' },
  喜悅之光: { src: '/icons/joy.png',     color: '#f6c453', note: '喜悅不是結果，而是你走在道上的自然狀態。' },
  發心之光: { src: '/icons/vow.png',     color: '#ef4444', note: '當你記起初心，力量自然湧現，步伐也會踏實而穩定。' },
}
...
export default function ResultPage() {
  ...
  // 2) 使用安全預設值，確保 useMemo 每次 render 都可執行
  ...
  // 3) 「不要在 hooks 之後再提前 return」
  //    以條件渲染的方式切換畫面內容（保持 hook 次序恆定）
  return (
    <main className="min-h-screen px-6 py-10 sm:px-10">
      {!mounted ? (
        // Loading 狀態（第一次 hydration 一致）
